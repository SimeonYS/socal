...
class SocalPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('socal.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `socal`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM socal WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO socal (date, title, link, content)"
